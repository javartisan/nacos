...
public class ServiceManager implements RecordListener<Service> {
    ...
    /**
     * Map(namespace, Map(group::serviceName, Service)). 按照服务的NameSpace维度进行划分存储服务的。优先会按照NameSpace寻找服务。
     */
    ...
    public void onDelete(String key) throws Exception {
        ...
        if (service != null) {
            service.destroy(); // 服务删除时候取消心跳发送任务
    ...
    public void createServiceIfAbsent(String namespaceId, String serviceName, boolean local, Cluster cluster)
        ...
        Service service = getService(namespaceId, serviceName);// 判断是否存在该namespace的serviceName服务
        if (service == null) {
            // 服务不存在，进行创建服务
            ...
            if (cluster != null) { // 集群不为空，将服务放到指定集群
                cluster.setService(service); // 将新创建的服务关联到集群上
    ...
    /**
     * Register an instance to a service in AP mode. if service of instance not exist then create it before register service
     *
    ...
    public void addInstance(String namespaceId, String serviceName, boolean ephemeral, Instance... ips)
        throws NacosException {
        // 生成instance key; 格式例如：com.alibaba.nacos.naming.iplist.ephemeral.public##DEFAULT_GROUP@@com.javartisan.SqlService
        String key = KeyBuilder.buildInstanceListKey(namespaceId, serviceName, ephemeral);

        Service service = getService(namespaceId, serviceName);

        synchronized (service) {
            List<Instance> instanceList = addIpAddresses(service, ephemeral, ips); // 将新的服务实例与现有服务实例合并
    ...
    public List<Instance> updateIpAddresses(Service service, String action, boolean ephemeral, Instance... ips)
        throws NacosException {
        // 根据 namespace serviceName 寻找现在的服务列表
    ...
    public Service getService(String namespaceId, String serviceName) {
        if (serviceMap.get(namespaceId) == null) {
            return null; // nameSpace不存在
        }
        return chooseServiceMap(namespaceId).get(serviceName); // namesace存在寻找服务
    }
